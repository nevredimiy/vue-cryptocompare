<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div class="hidden fixed w-100 h-100 opacity-80 bg-purple-800 inset-0 z-50 items-center justify-center">
    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input v-model="ticker" @keydown.enter="add()" @input="handleInput" @focus="isRepetitiveTicker = false"
                @keydown.left="isRepetitiveTicker = false" ref="searchTicker" type="text" name="wallet" id="wallet"
                class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
                placeholder="Например DOGE" />

            </div>
            <div v-if="filteredCoins.length" class="flex bg-white shadow-md p-1 rounded-md flex-wrap">
              <span @click="add(coin)" v-for="coin in filteredCoins.slice(0, 4)" :key="coin"
                class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
                {{ coin }}
              </span>
            </div>
            <div v-show="isRepetitiveTicker" class="text-sm text-red-600">Такой тикер уже добавлен</div>
          </div>
        </div>
      ...
      <template v-if="tickers.length">
        ...
        <div class="flex gap-4 bg-slate-300 px-6">
          ...
          <div class="flex gap-4 px-4">
            ...
            <button v-if="hasNextPage" @click="page + 1"
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div v-for="t in filteredTickersList()" :key="t.name" @click="handleSelectTicker(t)" :class="{ 'border-4': selectTicker === t }"
            ...
            <button @click.stop="handleDeleteTicker(t)"
      ...
      <section v-if="selectTicker" class="relative">
        <h3 class="text-lg leading-6 font-medium text-gray-900 my-8">
          {{ selectTicker && selectTicker.name }} - USD
        </h3>
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div v-for="(bar, idx) in normilizeGraph()" :key="idx" :style="{ height: bar + '%' }"
...
export default {
  ...
  created() {
    fetch('https://min-api.cryptocompare.com/data/all/coinlist?summary=true')
      ...
      .then(data => this.coins = Object.values(data.Data));

    const tickerListFromStorage = localStorage.getItem("cryptonomicon-list");
    if (tickerListFromStorage) {
      this.tickers = JSON.parse(tickerListFromStorage);
  ...
  methods: {
    filteredTickersList() {
      ...
      const filterList = this.tickers.filter(t => t.name.includes(this.filter));
    ...
    subscribeToUpdate(tickerName) {
      setInterval(async () => {
        ...
        this.graph.push(this.selectTicker?.find(ticker => ticker.name === tickerName).price);
    ...
    handleInput() {
      this.isRepetitiveTicker = false;
      this.filteredCoins = [];
      this.coins.forEach(coin => {
        if (coin.FullName.includes(this.ticker.toUpperCase())) {
          this.filteredCoins.push(coin.Symbol);
        }
      })
    },
    add(coinName = '') {
      if (coinName === '' && this.ticker === '') return;
      const currentTickerName = (coinName === '') ? this.ticker.toUpperCase() : coinName;
      ...
      if (this.coins.find((coin) => currentTickerName === coin.Symbol) === undefined) {
        this.ticker = '';
        this.$refs.searchTicker.focus()
        return;
      }

      if (this.tickers.find(t => t.name === currentTicker.name)) {
        this.isRepetitiveTicker = true;
        this.ticker = currentTicker.name;
        return;
      };

      if (this.filteredCoins.find(() => this.filteredCoins === currentTicker.name)) {
        this.isRepetitiveTicker = true;
        this.ticker = currentTicker.name;
        return;
      }
    ...
    normilizeGraph() {
      ...
      if (minValue === maxValue && this.graph.length) return this.graph.map(price => 50);
